...
namespace MeetAndTalk
{
    public class DialogueGetData : MonoBehaviour
    {
        ...
        {
            // Znajd� wszystkie linki, kt�re pasuj� do podanego w�z�a na podstawie BaseNodeGuid
            var matchingLinks = dialogueContainer.NodeLinkDatas
                .Where(link => link.BaseNodeGuid == _baseNodeData.NodeGuid)
                .ToList();

            // Sprawd�, czy znaleziono jakiekolwiek dopasowania
            if (matchingLinks.Count == 0)
            {
                return null; // Brak pasuj�cych w�z��w, zwr�� null
            }

            // Je�li jest wi�cej ni� jeden dopasowany link, losuj jeden z nich
            NodeLinkData selectedLink = matchingLinks.Count == 1
                ? matchingLinks[0] // Je�li jest tylko jeden, wybierz go
                : matchingLinks[UnityEngine.Random.Range(0, matchingLinks.Count)]; // W przeciwnym razie losuj

            // U�yj TargetNodeGuid z wybranego linku, aby pobra� odpowiedni w�ze�
        ...
        public class ConditionClass
        {
            ...
            public VisualElement CreateVisualElement()
            {
                ...
                // G��wny kontener dla element�w UI
