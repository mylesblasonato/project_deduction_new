        ...
        protected override void OnUpdate(float time, float previousTime) {
            if ( source != null ) {
                ...
                if ( !string.IsNullOrEmpty(subtitlesText) ) {
                    ...
                    var subs = subtitlesText;
                    if ( track.useSubtitlesTypewritterEffect ) {
                        var inv = Mathf.FloorToInt(Mathf.Lerp(0f, (float)subtitlesText.Length, time / ( length - 1f )));
                        subs = subtitlesText[..inv];
                    }

                    var lerpColor = subtitlesColor;
                    lerpColor.a = weight;
                    DirectorGUI.UpdateSubtitles(string.Format("{0}{1}", parent is ActorAudioTrack ? ( actor.name.Replace("(Clone)", string.Empty) + ": " ) : string.Empty, subs), lerpColor);
                }
            }
        }
