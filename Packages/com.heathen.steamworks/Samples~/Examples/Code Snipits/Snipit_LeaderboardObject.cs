...
namespace MyProperlyFormedNamespaceName
{
    ...
    public class Snipit_LeaderboardObject : MonoBehaviour
    {
...
#if UNITY_EDITOR
        //You can create a reference to a board in any inspector exposed script
        //for example the below will make a slot that appears in the inspector allowing you
        //to drag and drop a board to it
        [SerializeField]
        private LeaderboardObject targetBoard;

        //Exposing the name as a simple field here so that the sniped code is more similar between the data and object versions
        private string apiName => targetBoard.apiName;
                ...
                targetBoard.UploadScore(score, details, uploadMethod, (callbackResult, ioError) =>
                {
                    if (!ioError)
                    {
                        if (callbackResult.Success)
                        {
                            ...
                            targetBoard.data.AttachUGC("tempFile", fileData, (ugcResult, ugcIoError) =>
                            {
                                if (!ugcIoError)
                                {
        ...
        public void Example_ReadUserScore()
        {
            ...
            if (targetBoard.Valid)
            {
                ...
                targetBoard.GetUserEntry((foundEntry, ioError) =>
                {
                    if (!ioError)
                    {
                        if (foundEntry != null)
        ...
        public void Example_ReadTopScores()
        {
            ...
            {
                //The last parameter as always with async calls to Steamworks is a delegate
                //The delegate includes the LeaderboardEntry which is the found entries if any, and a bool indicating if an IO Error had occurred ... true is bad not good
                targetBoard.data.GetTopEntries(entriesToRead, targetBoard.maxDetailEntries, (entriesFound, ioError) =>
                {
                    if (!ioError)
                    {
